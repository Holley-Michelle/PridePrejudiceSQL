...
-- Lesson 3 - Joins
-- now that we have these tables created and populated lets practice joining tables. 
-- Joining the character_demographics table to the life_events (join on story_id) will give us a "sparknotes" of the story. 
-- We've gone over joins and I'll leave the notes from that lesson in your reference folder in this instance we'll be using a left join
...
GROUP BY 
  ...
  -- END of Lesson 3 - Joins
  -- Leson 4 - Queries 
  -- Querying is the most common use of SQL so we'll spend some more time here (Structured QUERY Language and all) 
-- so let's practice some custom queires 
-- Query 1 
-- Return charcters who live at Netherfield 
SELECT c.first_name, c.last_name #since we want a list of character names 
FROM character_demographics c #since this is the table that hosts the names
JOIN income_or_dowry i ON c.story_id = i.story_id # in this instance we need need the household id aligned to character as it is in the income_or_dowry table
JOIN household_details h ON i.household_id = h.household_id # household_details has the household name field we want if we had included name in the income or dowry table we
# wouldn't need 2 joins
